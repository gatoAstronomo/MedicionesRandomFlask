...
def imprimirMedicion(medicion):
    epoch_time, value = devolverDatos(medicion)
    print(f'{formatEpochTime(epoch_time)} {value}ºC')

def graficarMediciones(mediciones):
    epoch_times = [medicion.get("epoch_time") for medicion in mediciones]
    values = [medicion.get("value") for medicion in mediciones]

    fig, ax = plt.subplots()
    ax.xaxis_date()
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d %H:%M:%S'))

    line, = ax.plot(epoch_times, values)
    ...
    def actualizar(i):
        try:
            ...
            if ultima_medicion.get("epoch_time") != mediciones[-1].get("epoch_time"):
                ...
                line.set_ydata([medicion.get("value") for medicion in mediciones])
                ax.relim()
                ax.autoscale_view()
                plt.ylim(min(values) - 2, max(values) + 2)
                plt.xlim(epoch_times[-len(epoch_times)//60], epoch_times[-1])
